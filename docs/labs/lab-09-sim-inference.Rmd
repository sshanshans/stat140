...
- Find the repo starting with `lab-09` and that has your team name at the end.
...
1.  What are the cases (observations) in this data set? How many cases (observations) are there in our sample?
...
You will use `ncbirths_white` for Exercises 3 and 4.

3. Are the conditions necessary for conducting statistical inference satisfied? Explain your reasoning.
...
5.  Make side-by-side boxplots displaying the relationship between `habit` and 
`weight`. What does the plot highlight about the relationship between these 
two variables?

6. Before moving forward, save a version of the dataset omitting observations where there are NAs for `habit`. You can call this version `ncbirths_habitgiven`.
...
8. Are the conditions necessary for conducting statistical inference satisfied? Explain your reasoning.

9. Run the appropriate hypothesis test, calculate the p-value, and interpret the results in the context of the data and the hypothesis test.

10. Construct and interpret a 95% confidence interval for the difference between the average weights of babies born to smoking and non-smoking mothers.
