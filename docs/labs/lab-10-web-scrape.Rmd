...
In order to complete this lab you will need [Selector Gadget extension](http://selectorgadget.com/) installed.
...
- Let's start with the names of breweries. Use the selector gadget to determine the path to the names of breweries, and create a vector containing 315 elements of type character which are the names of the breweries.
...
- Scrape brewery type, number of beers brewed at each brewery, year when brewery first opened ("est.")s, and URL. Save these as vectors called `types`, `counts`, `ests`,  and `urls`, respectively. Additionally, create a vector called `status` that indicates whether each brewery is active or closed.

- Create a data frame (`tibble`) called `ncbreweries` with column names shown in the table above.

- Save this data frame as `ncbreweries.csv` using the `write_csv` function into the `/data` folder: `write.csv(ncbreweries, path = "data/ncbreweries.csv")`.
...
Repeat what you did above (potentially with some modifications) to create a similar data frame (with the same columns) for a different state of your choosing. Save the result in a csv file with the name of the state you chose, e.g. `cabreweries.csv`.
