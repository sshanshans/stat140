...
subtitle: "Are LaQunitas always next to Denny's?"
date: "due Mon, Feb 4 at 11:59p"
output: 
  tufte::tufte_html:
...
- Go to the course organization on GitHub: [https://github.com/STA199-Sp19](https://github.com/STA199-Sp19)
...
In this lab we will work with the `tidyverse` package, so we can load it in the **console**.
...
Each observation in this dataset represents a state, including DC. In addition to the name of the state, we have the two-letter abbreviation and the geographic area of the state (in square miles).
...
4. Now take a look at the data. What are some ways of determining whether or not either establishment has any locations outside the US using only the data (and not the websites). Don't worry about whether you know how to implement this, just brainstorm some ideas. Write down at least one idea as your answer, but you're may write down a few options too.
